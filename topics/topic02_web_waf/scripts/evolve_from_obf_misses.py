#!/usr/bin/env python3
"""从 eval_obf_misses.jsonl 诚实增量重训 RF（合并原训练集）。"""
...
def main() -> None:
    ...
    if not misses_path.exists():
        print(json.dumps({"updated": False, "reason": "misses file missing"}))
        return
    ...
    with misses_path.open(encoding="utf-8") as f:
        for line in f:
            ...
            if not label or label == "Normal" or not payload:
                continue
            record = {
                "url": f"http://evolve.local/?p={payload[:200]}",
                "true_label": label,
                "predicted": "Normal",
                "payload": payload,
                "source": row.get("source", "eval_miss"),
            }
